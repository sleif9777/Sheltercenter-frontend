...
import { deltaToPlainText, extractWildcardsFromDelta, replaceWildcardsInDelta } from "../../utils/QuillUtils"
...
export function MessageForm({
	...
	const handleSubmit: FormSubmitHandler<SendMessageRequest> = useCallback(
		async (req: SendMessageRequest) => {
			...
			if (adoptionID) {
				const resp = await new PendingAdoptionsAPI().AddUpdate(adoptionID, deltaToPlainText(updatedReq.message), adoptionSubject ?? updatedReq.subject ?? "")
				if (resp?.status && resp.status >= 200 && resp.status < 300) {
					showToast({ level: MessageLevel.Success, message: "Message sent!" })
					onSuccess?.()
				} else {
					showToast({ level: MessageLevel.Error, message: "Failed to send message. Please try again." })
				}
				return
			}
